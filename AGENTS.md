...
- `packages/esm-wrapper` — a small `tsdown`-built ESM boundary around DocSearch. `Search.astro` dynamically imports its built output instead of mounting DocSearch as an Astro React island. **Keep this wrapper**: Vite/Rolldown can misdetect DocSearch v5's React and CommonJS entry points, causing duplicate-looking React module requests, `_jsxDEV is not a function`, or missing `use-sync-external-store` named exports during development. React, React DOM, and DocSearch stay external in `tsdown.config.ts`; Vite excludes the DocSearch ESM entry points and explicitly pre-bundles the `use-sync-external-store` shims with `needsInterop`. `predev` builds the wrapper, then `dev:esm` watches it alongside Astro.
...
| --------------------------------- | --------------------------------------------------------------- |
| `pnpm dev`                        | Build client packages, then watch ESM wrapper and run Astro.    |
