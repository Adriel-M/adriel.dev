...
Built with `pkgroll` into `dist/atom-style.js`. `apps/site` imports the built file with
...
string. pkgroll can't import a `.svg`/`.txt`/`?raw` asset (only `.json`), so rather than
contort the source the markup just lives inline — it's tiny.
